...
class Model(nn.Module):
    def __init__(self, input_channels=3, latent_channels=8, num_embeddings=1024):
        ...
        # Codebook
        self.codebook = CodebookModel(num_embeddings, latent_channels)
    ...
    def decode(self, h, return_multiple=False):
        z_q, vq_loss, indices, perplexity = self.codebook(h)
        h = self.post_quant_conv(z_q)
        if return_multiple:
            return self.decoder(h), vq_loss, perplexity
        return self.decoder(h)

    def forward(self, x):
        z_q = self.encode(x)
        x_recon, vq_loss, perplexity = self.decode(z_q, True)
